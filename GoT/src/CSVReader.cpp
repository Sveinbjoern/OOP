...
OrderBookEntry CSVReader::CheckValidData__ParseOBE(std::string& line, OrderBook& orderBook, const bool add )
{
    ...
    if (tokens.size() == 5)
    {
        ...
        {
            s = tokens[0];
        }else {throw std::invalid_argument( "Length of time input is not valid. should be 26." );}
        ...
        OrderBook::addToSetOfProducts(orderBook,tokens[1]);
        product.set(tokens[1], orderBook);
        // std::cout << "tokens[1]"<< (tokens[1] == product.get_type());

        // product = tokens[1];
        // CSVReader::tokenize(tokens[1], '/');
    ...
    OrderBookEntry OBE( s ,product,OBT ,price,amount, false );
    // if(n >= 2) break;
    //  std::cout << std::fixed << "p" << price;
    // std::cout << OBE.product.get_type();
    return OBE;
