    ...
    */
    auto lamMinMaxString = [&](std::string minOrMax)
    {
        ...
        } else
        {
            ...
            // std::cout << "prodTest GOT " << tokens[1] << std::endl;
            // std::cout << "prodTest GOT " << prod.get_type() << std::endl;
           


            if (tokens.size() == 2 )
            {
                double bid = orderBook.getHighOrLowPriceBid(minOrMax, prod);
                int numberOfBids = orderBook.n;
                double ask = orderBook.getHighOrLowPriceAsk(minOrMax, prod);
                int numberOfAsks = orderBook.n;
                ...
                returnString.append(".\n");
                returnString.append("From a total of ");
                returnString.append(to_string(numberOfBids));
                returnString.append(" bids.\n");
                ...
                returnString.append(".\n");
                returnString.append("From a total of ");
                returnString.append(to_string(numberOfAsks));
                returnString.append(" bids.\n");
    ...
    /**lambda functions for the function GameOfTrades::mainInput  
     * creates the return string for inputs avg
    */
    auto lamAverageString = [&]()
    {
        // std::cout << "runing lamda for minMax";
        std::string returnString;
        std::string example = "Example: ";
    ...
    if (tokens.size() > 0)
    {
        ...
        {
            overText = "Current timestep: ";
            overText.append(orderBook.getCurrentTimeString());
            overText.append("\nThe ");
            overText.append(to_string(orderBook.getCurrentTimeNumberIndex()+1));
            overText.append(".th timestep of ");
            overText.append(to_string(orderBook.getCurrentTimeNumberTotal()));
            overText.append(" total timesteps.\n");

            return overText;
        } else if (tokens[0] == "STEP")
        {
            overText = "Moving forward a timeStep.\n";
            overText.append("From: ");
            overText.append(orderBook.getCurrentTimeString());

            std::string test = orderBook.goToNextTimestep();
            if (test.back() == '0')
            {
                return test;
            }
