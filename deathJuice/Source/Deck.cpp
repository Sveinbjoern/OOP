...
Deck::Deck(SoundElement& _soundElement,
	...
	bool _reverseLayout,
	juce::Colour _deckColour) : soundElement(_soundElement),
	reverseLayout(_reverseLayout),
	waveform(formatManagerToUse, cacheToUse, _soundElement, _deckColour),
	playlistComponent(_playlistComponent),
	deckColour(_deckColour)
	//currentTrack(Track{})
{
	
	soundElementHasBeenSent = false;

	heightSteps = 8;
	widthSteps = 8;
	currentTrack = nullptr;

	vinyl.setupVinyl(currentTrack, deckColour, &soundElement);
	...
	gainSlider.setColour(juce::Slider::thumbColourId, deckColour);
	//WAIT
	...
	speedSlider.setColour(juce::Slider::trackColourId, deckColour);
	//speedSlider.setTextBoxIsEditable(false);
	speedSlider.addListener(this);
	speedSlider.setNumDecimalPlacesToDisplay(3);

	addAndMakeVisible(speedLabel);
	speedLabel.setText("SPD", juce::NotificationType::dontSendNotification);
	...
	posSlider.setColour(juce::Slider::thumbColourId, deckColour);
	//addAndMakeVisible(posLabel);
	//posLabel.setText("POS", juce::NotificationType::dontSendNotification);
	//posLabel.attachToComponent(&posSlider, true);
	//posLabel.setJustificationType(juce::Justification::topLeft);
	//addAndMakeVisible(vinyl);
...
void Deck::paint(juce::Graphics& g)
{
	...
	*/
	...
	float widthStep = getWidth() / widthSteps;
	float heightStep = getHeight() / heightSteps;
...
void Deck::resized()
{
	...
	float buttonHeight = getHeight() - halfHeightStep * 2.2;

	float vinylHeight = heightStep * (heightSteps - 2);
	float vinylWidth = widthStep * (widthSteps - 2);



	playButton.setBounds(halfWidth - widthStep * 2, buttonHeight, widthStep, halfHeightStep);
	stopButton.setBounds(halfWidth - widthStep, buttonHeight, widthStep * 2, halfHeightStep);
	loadButton.setBounds(halfWidth + widthStep, buttonHeight, widthStep, halfHeightStep);

	if (reverseLayout)
	{
		float vinylDiam;
		float vinylOffset;
		if (vinylHeight > vinylWidth)
		{
			vinylDiam = vinylWidth - 4;
			vinylOffset = (vinylHeight - vinylDiam) / 2;
			vinyl.setBounds(widthStep * 2 + 2, heightStep + vinylOffset + 2, vinylDiam, vinylDiam);
		}
		else
		{
			vinylDiam = vinylHeight - 4;
			vinylOffset = (vinylWidth - vinylDiam) / 2;
			vinyl.setBounds(widthStep * 2 + vinylOffset + 2, heightStep + 2, vinylDiam, vinylDiam);
		}
		speedSlider.setBounds(0, 30, widthStep, getHeight() - 30);
		gainSlider.setBounds(widthStep, heightStep, widthStep, getHeight() - heightStep);
		posSlider.setBounds(widthStep, 0, widthStep * (widthSteps - 1), heightStep);
	...
	else
	{
		...
		if (vinylHeight > vinylWidth)
		{
			vinylDiam = vinylWidth - 4;
			vinylOffset = (vinylHeight - vinylDiam) / 2;
			vinyl.setBounds(2, heightStep + vinylOffset + 2, vinylDiam, vinylDiam);
		}
		else
		{
			vinylDiam = vinylHeight - 4;
			vinylOffset = (vinylWidth - vinylDiam) / 2;
			vinyl.setBounds(vinylOffset + 2, heightStep + 2, vinylDiam, vinylDiam);
		}
		gainSlider.setBounds(widthStep * (widthSteps - 2), heightStep, widthStep, getHeight() - heightStep);
		speedSlider.setBounds(widthStep * (widthSteps - 1), 30, widthStep, getHeight() - 30);

		posSlider.setBounds(0, 0, widthStep * (widthSteps - 1), heightStep);
...
{
	if (!soundElementHasBeenSent)
	{
		soundElementHasBeenSent = true;
		return soundElement;
	}
}
...
void Deck::buttonClicked(juce::Button* button)
{
	...
	else if (button == &loadButton)
	{
		...
		juce::FileChooser chooser{ "Select a sound file(s) to play..." };
		...
		if (chooser.browseForMultipleFilesToOpen())
		{
			...
			for (int i = results.size(); i >= 0; --i)
			{
				...
				if (soundElement.loadFile(results[i]))
				{

					//    DBG("Loadbutton chooser.getResult");
					if (playlistComponent.addToSetOfTracks(results[i]))
						addNewTrack(results[i]);

					setDeckAfterLoadingTrack(playlistComponent.getTrack(results[i]));
...
void Deck::sliderValueChanged(juce::Slider* slider)
{
	...
	{
		soundElement.setSpeed(slider->getValue());
	}
...
{
	Track* track = playlistComponent.getSearchedTrack(trackNumber);
...
void Deck::resetDeck()
{
	stopDeck();
	currentTrack = nullptr;
	vinyl.loadInTrack(currentTrack);
